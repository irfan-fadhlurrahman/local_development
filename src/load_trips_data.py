...
import pandas as pd
from utils.db_connection import PostgresConnection, get_database_credentials
...
class NYCGovTaxiToPostgresPipeline:
    ...
    def __init__(self, datetime_columns: list):
        self.engine = PostgresConnection(db_conn=get_database_credentials()).engine()
...
def run(params):
    ...
    print(f"Extracting the data from {url}")
    ...
    for df in pipeline.extract(url, chunksize=chunksize):
        ...
        print(f"Ingestion of {len(df)} rows to table {table_name} took {end-start:.2f} seconds")
    
    print(f"Ingestion to table {table_name} finished")
